...
import random
import os
# Try to import python-magic; fall back to mimetypes if libmagic is unavailable
try:
    import magic  # type: ignore
    _MAGIC_AVAILABLE = True
except Exception:  # ImportError or runtime errors loading libmagic
    import mimetypes
...
def validate_file_type(file, allowed_types):
    ...
    try:
        ...
        if _MAGIC_AVAILABLE:
            ...
            mime_type = magic.from_buffer(file_content, mime=True)  # type: ignore[attr-defined]
        else:
            # Fallback: best-effort based on filename
            mime_type, _ = mimetypes.guess_type(getattr(file, "name", ""))
        return bool(mime_type) and mime_type in allowed_types
